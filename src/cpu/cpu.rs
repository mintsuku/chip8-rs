...
impl CPU {
    ...
    pub fn fetch(&mut self) -> u16 {
        ...
        if nibble != 0x3000 && nibble != 0x4000 && nibble != 0x5000 && nibble != 0x9000 {
            self.pc += 2;
        }
    ...
    fn skip_if_vx_equal(&mut self, instruction: u16) {
        ...
        println!("gpr value: {} - comparing to: {}", self.gprs[vx_register as usize], value as u8);
    ...
    fn draw(&mut self, instruction: u16, frame_buffer: &mut [u8]) {
        ...
        let x = self.gprs[vx as usize] as usize;
        let y = self.gprs[vy as usize] as usize;
        ...
        if let Some(sprite_bytes) = self.read_bytes(self.index_reg as usize, n as usize) {
            for (row, &byte) in sprite_bytes.iter().enumerate() {
                for col in 0..8 {
                    ...
                    let fb_index = ((y + row) * 64 + (x + col)) as usize;
                    if pixel == 1 {
